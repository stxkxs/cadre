...
const statusIcon = (status: string) => {
  switch (status) {
    case 'running': return <Loader2 className="h-4 w-4 animate-spin text-[#0AEFB7]" />;
    ...
    <div className="space-y-4 h-[calc(100vh-7rem)] flex flex-col -m-6 p-6">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground font-display">Run Monitor</h1>
            <div className="flex items-center gap-2 mt-0.5">
              <Badge variant={statusBadgeVariant(overallStatus)}>{overallStatus}</Badge>
              {!isDone && isConnected && (
                <span className="flex items-center gap-1 text-xs text-[#0AEFB7]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#0AEFB7] animate-pulse" />
                  Live
                </span>
              )}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-6 text-sm">
            <div className="flex items-center gap-1.5 text-muted-foreground">
              <Zap className="h-4 w-4 text-[#0AEFB7]" />
                  ...
                  <div
                    ...
                    className={`rounded-lg border px-4 py-3 transition-all ${
                      state.status === 'running'
                        ? 'border-[#0AEFB7]/50 bg-[#0AEFB7]/5 node-scan'
                  ...
                  <div className="rounded-lg bg-input p-4 font-display text-sm text-muted-foreground min-h-[200px] whitespace-pre-wrap">
                    {combinedOutput || (
                      <span className="text-dim">
                        ...
                        {!isDone && <span className="inline-block w-2 h-4 bg-[#0AEFB7] animate-pulse ml-1" />}
                      </span>
                    )}
                  </div>
                </ScrollArea>
